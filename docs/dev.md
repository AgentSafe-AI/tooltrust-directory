...
1. **Discover** — runs `cmd/crawler` to search GitHub for MCP servers (50+ stars) and Smithery-native servers (10+ uses), prefers releases/tags/package versions, and falls back to the default-branch HEAD SHA when a repo has no formal version metadata.

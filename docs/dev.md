...
1. **Discover** — runs `cmd/crawler` to search GitHub for MCP servers (50+ stars) and Smithery-native servers (10+ uses), prefers releases/tags/package versions, and only falls back to the default-branch HEAD SHA for curated seed/override repos that have no formal version metadata.
